...
def cam_trans(p, tx, ty, tz):
    cam_t = [[1, 0, 0, tx],
             ...
             [0, 0, 0, 1]]
    p = [p[0], p[1], p[2], 1]
    camera_out = np.matmul(cam_t, p)
    return camera_out



def obrot_zad_2(punkt, kat, srodek, p0, os):
    x = srodek[0]
    y = srodek[1]
    z = srodek[2]
    ...
    if os == 'x':
        ...
        M = [[x + p0[0],((y * math.cos(kat)) - (z * math.sin(kat))) + p0[1],((y * math.sin(kat)) + (z * math.cos(kat))) + p0[2]],
         [((x * math.cos(kat)) + (z * math.sin(kat))) + p0[0],y + p0[1],((z * math.cos(kat)) - (x * math.sin(kat))) + p0[2]],
         [((x * math.cos(kat)) - (y * math.sin(kat))) + p0[0],((x * math.sin(kat)) + (y * math.cos(kat))) + p0[1],z + p0[2]]]

        M = np.array(M)
        nowypunkt = np.array(punkt)
        nowypunkt = nowypunkt - p0
        nowypunkt = M @ nowypunkt.T
        nowypunkt = nowypunkt + np.array(p0)
        nowypunkt = nowypunkt.T
...
def szescian_zad_1(dlugoscboku, srodek, kat,p0, R, G, B, os,x1,x2,x3):
    pkt = [[srodek[0] - dlugoscboku, srodek[1] - dlugoscboku, srodek[2] - dlugoscboku],
           [srodek[0] + dlugoscboku, srodek[1] - dlugoscboku, srodek[2] - dlugoscboku],
           [srodek[0] + dlugoscboku, srodek[1] + dlugoscboku, srodek[2] - dlugoscboku],
           [srodek[0] - dlugoscboku, srodek[1] + dlugoscboku, srodek[2] - dlugoscboku],
    ...
    for i in range(8):
        pkt[i] = obrot_zad_2(pkt[i], kat, srodek,p0, os)
...
while True:
    ...
    if key_out == 1:
        szescian_zad_1(5, [0, 0, 30], kat,[1,1,1], 1, 1, 1, 'x',x1,x2,x3)
    elif key_out == 2:
        szescian_zad_1(5, [0, 0, 30], kat,[1,1,1], 1, 1, 1, 'y',x1,x2,x3)
    elif key_out == 3:
        szescian_zad_1(5, [0, 0, 30], kat,[1,1,1], 1, 1, 1, 'z',x1,x2,x3)
    elif key_out == 4:
        prostopadloscian_1(2, 5, 7, [2, 2, 30], 0.1, 0.1, 0.1, 1, 1, 1)
    elif key_out == 5:
        szescian_zad_1(5, [0, 0, 30], kat,[-2.5, 2.5 ,30], 1, 1, 1, 'prosta', x1, x2, x3)
        szescian_zad_1(5, [0, 0, 30], kat,[-4.33, 4.33, 1], 1, 1, 1, 'prosta', x1, x2, x3)
