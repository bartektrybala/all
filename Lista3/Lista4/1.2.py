...
def prostokat(p4, p2, R, G, B):  # rysowanie trojkata w 3D
    p1 = [p4[0], p2[1], 0]
    p3 = [p2[0], p4[1], 0]
    odcinek3D(p1, p2, R, G, B)
    odcinek3D(p2, p3, R, G, B)
    odcinek3D(p3, p4, R, G, B)
    odcinek3D(p4, p1, R, G, B)
...
def handler(key, x, y):
    ...
    if key[0] == ord('o'):
        odcinek3D([1, 1, 1], [2, 2, 1], 1.0, 1.0, 1.0)
    elif key[0] == ord('t'):
        trojkat([2, 4, 0], [5, 7, 0], [8, -1, 0], 1.0, 1.0, 1.0)
    elif key[0] == ord('p'):
        prostokat([-5, 5, 0], [5, -6, 0], 1.0, 1.0, 1.0)
    elif key[0] == ord('d'):
        prostopadloscian([6, 3, 2], [-2, 4, 30], 0.2, 0.1, 0.2, 1.0, 1.0, 1.0)
    paint()
    glutMainLoopEvent()
